...
def read_amazon_review_raw_data_and_split(dataset_path, word_dim=100):
    ...
    if os.path.exists(dataset_info_path):
        ...
        train_data, valid_data, test_data = split_data(data)  # @ss
...
def split_data(data: pd.DataFrame) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
    ...
    # 16962/2=8481 从min里挑8481,从most里挑选8481，   从min里挑选8481/24125=35.15%的评级，从most里挑选8481/145498=5.83%的评级
    #需要在测试集里，冷门和热门出现的概率是一样多的                     # sum([sum(list(x.values())) for x in list(user_less_min_raing.values()) if x != {}])/2
